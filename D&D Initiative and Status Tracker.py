...
from utils.health_status_helpers import (
    delayed_check_health_status,
    get_status_string_and_color,
    get_health_status_color_indicator,
)
...
class InitiativeTracker(customtkinter.CTk):
    def __init__(self):
        ...
        self.characters = {
            "Nareina": {
                ...
                "armor_class": "15",
                "saving_throws": "Str +1, Dex +5, Con +1, Int +0, Wis +2, Cha +1",
            },
            "Arantarr": {
        ...
        self.participant_initiative_entry = customtkinter.CTkEntry(
            self.sidebar_frame, placeholder_text="Player Initiative"
        )
        self.participant_initiative_entry.grid(row=4, column=0, padx=20, pady=10)

        self.participant_health_entry = customtkinter.CTkEntry(
            self.sidebar_frame, placeholder_text="Player Health"
        )
        self.participant_health_entry.grid(
            row=5, column=0, padx=20, pady=10, sticky="n"
        )
        ...
        self.initial_values = (
            {}
        )  # dictionary with the following strucutre: {"name": (initiative, maximum_health, monster_type)}
        self.status_lists = {}
        self.current_health = {}
    ...
    def add_player(self):
        ...
        sorted_initial_values = sorted(
            self.initial_values.items(), key=lambda x: x[1]["initiative"], reverse=True
        )
    ...
    def update_initiative_text(self, sorted_initial_values):
        self.main_frame = customtkinter.CTkScrollableFrame(
            self, width=1000, height=1000, corner_radius=0
        )
        self.main_frame.grid(
            row=0, column=1, sticky="nsew", padx=30, pady=20, rowspan=5
        )
        self.main_frame.grid_rowconfigure(5, weight=1)
        ...
        for _, attributes in sorted_initial_values:
            for key in attributes.keys():
                if key not in self.header_mappings and key not in [
                    "initiative",
                    "health",
                    "type",
                    "skills",
        ...
        for col, header in enumerate(dynamic_headers):
            customtkinter.CTkLabel(master=self.main_frame, text=header).grid(
                column=col, row=0, sticky="w", padx=5, pady=5
            )
        ...
        for participant, attributes in sorted_initial_values:
            ...
            customtkinter.CTkLabel(master=self.main_frame, text=participant).grid(
                column=0, row=row_count, sticky="w", padx=5, pady=5
            )

            # Display Initiative value
            customtkinter.CTkLabel(
                master=self.main_frame, text=attributes["initiative"]
            ).grid(column=1, row=row_count, padx=5, pady=5)

            # Display Health value
            customtkinter.CTkLabel(
                master=self.main_frame, text=attributes["health"]
            ).grid(column=2, row=row_count, padx=5, pady=5)

            # Health entry widget
            health_entry = tk.StringVar()
            health_entry_widget = customtkinter.CTkEntry(
                master=self.main_frame, textvariable=health_entry, width=50
            )
            health_entry_widget.insert(
                0, str(self.current_health.get(participant, attributes["health"]))
            )
            ...
            status_label = customtkinter.CTkButton(
                ...
                text="HP",
                width=60,
                command=lambda p=participant: open_status_window(
                    p, self.current_health, self.initial_values, self.status_lists
                ),
            )
            ...
            status_color = get_health_status_color_indicator(
                self.current_health.get(participant, attributes["health"]),
                attributes["health"],
            )
            ...
            health_entry.trace_add(
                ...
                callback=lambda mode, name, index, p=participant, he=health_entry, sl=status_label: (
                    delayed_check_health_status(
                        self, p, he, self.current_health, sl, self.initial_values
                    )
                ),
            )
            ...
            customtkinter.CTkButton(
                ...
                text="Delete",
                width=60,
                command=lambda p=participant: self.delete_entry(p),
            ).grid(column=4, row=row_count, padx=5, pady=5)
            ...
            customtkinter.CTkButton(
                ...
                text="Hide",
                width=60,
                command=lambda p=participant: roll_stealth(
                    p, self.initial_values, refresh_callback=self.refresh_display
                ),
            ).grid(column=5, row=row_count, padx=5, pady=5)
            ...
            if participant not in self.characters.keys():
                customtkinter.CTkButton(
                    ...
                    text="Statblock",
                    width=60,
                    command=lambda p=participant: self.on_statblock_click(p),
                ).grid(column=6, row=row_count, padx=5, pady=5)

            # Dynamically add additional attributes as columns if they exist
            for col_offset, key in enumerate(
                sorted(self.header_mappings.keys()), start=7
            ):  # Start after basic columns
                ...
                if key == "current_stealth":
                    if value != "-" and participant not in self.detected_characters:
                        ...
                        stealth_color = "transparent"
                    customtkinter.CTkLabel(
                        master=self.main_frame,
                        text=value if value else "-",
                        fg_color=stealth_color,
                    ).grid(column=col_offset, row=row_count, padx=5, pady=5)
                elif key == "passive_perception":
                    if value != "-" and participant in self.spotting_characters:
                        percecption_color = "green"
                    else:
                        percecption_color = "transparent"
                    customtkinter.CTkLabel(
                        master=self.main_frame,
                        text=value if value else "-",
                        fg_color=percecption_color,
                    ).grid(column=col_offset, row=row_count, padx=5, pady=5)
                    ...
                    customtkinter.CTkLabel(
                        master=self.main_frame,
                        text=value if value else "-",
                        fg_color=get_condition_color(value),
                    ).grid(column=col_offset, row=row_count, padx=5, pady=5)
    ...
    def on_statblock_click(self, participant):
        ...
        if self.firefox_driver:
            try:
                ...
                self.firefox_driver.switch_to.window(
                    self.firefox_driver.window_handles[-1]
                )

                # Bring the Firefox window to the front
                window = gw.getWindowsWithTitle("Mozilla Firefox")[
                    0
    ...
    def add_monster(
        ...
        highest_monster_count = get_starting_monster_count(
            monster_name, self.initial_values
        )
    ...
    def refresh_display(self):
        for widget in self.main_frame.winfo_children():
            widget.destroy()
        self.detected_characters, self.spotting_characters = check_for_detection(
            self.initial_values
        )
        sorted_initial_values = sorted(
            self.initial_values.items(), key=lambda x: x[1]["initiative"], reverse=True
        ...
        self.update_initiative_text(
            sorted_initial_values
        )  # Ensure you pass the right sorted_initial_values
