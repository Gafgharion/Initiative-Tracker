...
def open_status_window(
    participant, current_health, initial_values, status_strings_list
):
    ...
    health_percentage = (
        (current_hitpoints / max_hitpoints) * 100 if max_hitpoints > 0 else 0
    )

    # Get health status string and color
    health_status, color = get_random_status_string(
        health_percentage,
        type=initial_values[participant]["type"],
        status_dict=status_strings_list,
    )
